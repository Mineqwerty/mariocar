...
if (gMarioState->starReqs[16] > 0 && gMarioState->starReqs[16] < 5) {
    ...
    if (pedTimer >= 450) {
        pedTimer = 0;
        gMarioState->starReqs[16] = 0;
    }
}

if (gMarioState->pos[0] > 20000 || gMarioState->pos[0] < -20000 || gMarioState->pos[2] > 20000 || gMarioState->pos[2] < -20000) {
    gMarioState->starReqs[24] = 1;
...
if (gCurrLevelNum == LEVEL_CASTLE_GROUNDS) {
    ...
    if (starTimer > 15) {
        print_text(120, 150, "-U");
    }
    if (starTimer > 25) {
        print_text(150, 150, "GOT");
    }
    if (starTimer > 35) {
        print_text(190, 150, "STAR-");
    }

    if (starTimer > 55) {
        
        starTimer = 0;
        gMarioState->starCutscene = 0;
        //func_80321080(0);
        //stop_background_music(sCurrentBackgroundMusicSeqId);
        //play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(5, SEQ_COUNT), 0);
    }
    }
    for (int i = 1; i<31; i++) {
        if (gMarioState->starReqs[i] >= starNumberNeeded[i] && gMarioState->starsCollected[i] == 0) {
            gMarioState->starsCollected[i] = 1;
            spawn_object_abs_with_rot(gMarioObject, 0, MODEL_STAR, bhvStar, gMarioObject->oPosX, gMarioObject->oPosY, gMarioObject->oPosZ, 0, 0, 0);
    ...
    if (gPlayer1Controller->buttonPressed & R_JPAD) {
        ...
        gMarioState->starReqs[23] += 1;
        musicStart = 1;
        if (gMarioState->starReqs[23] > 4) {
            gMarioState->starReqs[23] = 0;
            gMarioState->starReqs[29] = 1;
        }
    }
...
if (musicStart == 1) {
    musicStart = 0;
    switch (gMarioState->starReqs[23]) {
        case 0: func_80321080(0); play_secondary_music(SEQ_STREAMED_CITY_FUNK, 255, 255, 0); //play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(5, SEQ_STREAMED_CITY_FUNK), 0);
        break;
        case 1: func_80321080(0); play_secondary_music(SEQ_STREAMED_OASIS, 255, 255, 0);//play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(5, SEQ_STREAMED_OASIS), 0);
        break;
        case 2: func_80321080(0); play_secondary_music(SEQ_STREAMED_FOREST, 255, 255, 0);//play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(5, SEQ_STREAMED_FOREST), 0);
        break;
        case 3: func_80321080(0); play_secondary_music(SEQ_STREAMED_SOMBER, 255, 255, 0);//play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(5, SEQ_STREAMED_SOMBER), 0);
        break;
        case 4: func_80321080(0); play_secondary_music(SEQ_STREAMED_PISS, 255, 255, 0);//play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(5, SEQ_STREAMED_PISS), 0);
        break;
    }
}

if (gMarioState->reverse == 0) {
    gMarioState->reverse = 1;
}
else if (gMarioState->reverse == -1) {
    gMarioState->starReqs[15] = 1;
}
...
if (gMarioState && gMarioObject && gMarioState->wheelSummon == 0) {
    spawn_object(m, MODEL_WHEELS, bhvClamShell);
    gMarioState->wheelSummon = 1;
}

    if (gPlayer1Controller->buttonDown & A_BUTTON || gPlayer1Controller->buttonDown & Z_TRIG || gPlayer1Controller->buttonDown & B_BUTTON) {
        if (gMarioState->carAccel < 50.0f) {
        gMarioState->carAccel += 2.0f;
        }
    }
    else if (gMarioState->carAccel > 0) {
        gMarioState->carAccel -= 0.5f;
    }
