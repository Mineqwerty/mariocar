...
void update_shell_speed(struct MarioState *m) {
    ...
    if (m->forwardVel <= 0.0f) {
        ...
        if (m->reverse > 0) {
        m->forwardVel += (m->carAccel/10) - (m->forwardVel / 28.0f);
        }
        else {
            m->forwardVel -= (m->carAccel/10) - (m->forwardVel / 28.0f);
        }
...
s32 act_riding_shell_ground(struct MarioState *m) {
    ...
    set_mario_animation(m, m->actionArg == 0 ? MARIO_ANIM_A_POSE : MARIO_ANIM_A_POSE);
    ...
    switch (perform_ground_step(m)) {
        ...
        case GROUND_STEP_HIT_WALL:
            ...
            m->forwardVel = -50.0f + -m->forwardVel * 2/3;
            m->carAccel = 0.0f;
            }
            m->starReqs[14] += 1;
